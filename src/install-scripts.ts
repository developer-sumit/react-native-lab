...
export async function installJDK() {
  ...
  if (process.platform === "win32") {
    ...
    try {
      execSync("sudo apt update && sudo apt install -y openjdk-17-jdk", {
        stdio: "inherit",
      });
      spinner.succeed(colors.green("OpenJDK installed successfully."));
    } catch (error) {
    ...
    if (!javaHomePath) {
      ...
      const javaHome = path.dirname(path.dirname(javaPath));
      try {
        execSync(`echo "export JAVA_HOME=${javaHome}" >> ~/.bashrc`, {
          stdio: "inherit",
        });
        execSync(`echo "export PATH=$PATH:${javaHome}/bin" >> ~/.bashrc`, {
          stdio: "inherit",
        });
...
export async function installAndroidStudio() {
  ...
  if (process.platform === "win32") {
    ...
    try {
      execSync("sudo apt update && sudo apt install -y android-studio", {
        stdio: "inherit",
      });
      spinner.succeed(colors.green("Android Studio installed successfully."));
    } catch (error) {
    ...
    try {
      execSync(`echo "export ANDROID_HOME=${androidHomePath}" >> ~/.bashrc`, {
        stdio: "inherit",
      });
      execSync(
